import { Corner, MatchRound } from '@prisma/client';
...
export interface GeneratedMatch {
  ...
  // Loser advancement (for bronze medal)
...
export class BracketGenerator {
  static generateSingleElimination(
    ...
    const athleteIds = sortedAthletes.map((athlete) => athlete.athleteId);
    const drawSize = this.nextPowerOfTwo(athleteIds.length);
    const totalRounds = Math.log2(drawSize);

    const slots: (string | undefined)[] = new Array(drawSize).fill(undefined);

    const seedPositions = this.generateSeedPositions(drawSize);
    ...
    while (currentSlots.length >= 2) {
      ...
      const round = this.getRound(totalRounds, roundIndex);
      ...
      if (previousRoundMatchSlots.length > 0) {
        previousRoundMatchSlots.forEach((previousSlot, index) => {
          const previousMatch = matches.find(
            (match) => match.bracketSlot === previousSlot
          );
          if (!previousMatch) return;

          const nextMatchIndex = Math.floor(index / 2);
          const nextMatchSlot = currentRoundMatchSlots[nextMatchIndex];

          previousMatch.nextSlot = nextMatchSlot;
          previousMatch.nextCorner = index % 2 === 0 ? Corner.RED : Corner.BLUE;
        });
      }

      previousRoundMatchSlots = currentRoundMatchSlots;
      currentSlots = new Array(currentSlots.length / 2).fill(undefined);
      roundIndex++;
    }
    ...
    // Connect semifinal losers to bronze match
    const semifinalMatches = matches.filter(
      (match) =>
        match.round === MatchRound.SEMI_FINAL ||
        match.round === MatchRound.SEMIFINAL
    );

    if (semifinalMatches.length === 2) {
      semifinalMatches[0].loserNextSlot = bronzeSlot;
      semifinalMatches[0].loserNextCorner = Corner.RED;

      semifinalMatches[1].loserNextSlot = bronzeSlot;
      semifinalMatches[1].loserNextCorner = Corner.BLUE;
    }
  ...
  private static getRound(
    ...
    if (roundsRemaining === 1) return MatchRound.FINAL;
    if (roundsRemaining === 2) return MatchRound.SEMI_FINAL;
    if (roundsRemaining === 3) return MatchRound.QUARTER_FINAL;
  ...
  private static generateSeedPositions(size: number): number[] {
    let positions = [1];
